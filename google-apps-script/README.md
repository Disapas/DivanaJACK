...
## อัปเดตสคริปต์ (เมื่อ Code.gs เปลี่ยนแปลง)

`Code.gs` ในโฟลเดอร์นี้อยู่ภายใต้ git แต่ **ไม่ได้เชื่อมกับ Google Apps Script โดยอัตโนมัติ** ทุกครั้งที่ไฟล์นี้ถูกแก้ไข (เช่น ตอนนี้ที่เพิ่มการแนบรูปถ่ายผู้สมัครลงในเอกสาร) คุณต้องคัดลอกเนื้อหาไปวางทับใน Apps Script editor เองอีกครั้ง:

1. เปิดโปรเจกต์ Apps Script เดิมที่ [script.google.com](https://script.google.com)
2. เลือกโค้ดทั้งหมดในไฟล์ แล้วลบ วางเนื้อหาใหม่จาก [`Code.gs`](./Code.gs) แทน
3. กด **Deploy > Manage deployments** เลือก deployment ที่ใช้งานอยู่ กด ✏️ (Edit) แล้วเปลี่ยน **Version** เป็น **New version** จากนั้นกด **Deploy**
   (การแก้โค้ดเฉยๆ โดยไม่สร้าง New version ใหม่ จะไม่มีผลกับ Web app URL ที่ใช้งานอยู่)
4. URL ของ Web app จะไม่เปลี่ยน ไม่ต้องแก้ `assets/js/config.js` ใหม่
